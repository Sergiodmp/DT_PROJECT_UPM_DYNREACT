...
import torch
import numpy as np
from decision_transformer.utils2 import evaluate_on_env, get_d4rl_normalized_score, get_d4rl_dataset_stats
...
def test(args):
    ...
    if args.env == 'env_va':
        eval_env_name = 'env_va'
        #from prueba_env import env_va
        from environment import env_va
        eval_env = env_va()      #instanciamos la clase
        eval_rtg_target = 100                    # Pequeño, porque los rewards se han programado muy pequeños. Se puede cambiar en el futuro
        eval_env_d4rl_name = f'env_va-{eval_dataset}-v2'
    ...
    for eval_chk_pt_name in eval_chk_pt_list:
        ...
        eval_chk_pt_path = os.path.join(eval_chk_pt_dir, eval_chk_pt_name)          #constructs the full path to the checkpoint file that will be loaded for evaluating the model.
        # load checkpoint
        ...
        results = evaluate_on_env(eval_model, device, context_len,
                                eval_env, eval_rtg_target, eval_rtg_scale,
                                num_test_eval_ep, eval_max_eval_ep_len,
                                eval_state_mean, eval_state_std, False)
        print(results['eval/avg_reward'])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--chk_pt_name', type=str,default='dt_env_va-medium-v2_model_24-01-15-20-47-46.pt')
